      ...
      <span>
        {easterEggs.includes(EasterEggs.CarWarranty) ? (
          <>
            ...
            <strong className='text-primary'>car's extended warranty</strong>.
          </>
        ) : (
          children
        )}
  ...
  return (
    <>
      {allChannels.length === 0 && sidebarView === 'spaces' && (
        <Notice>
          <Trans t={t}>
            <strong className='text-primary'>Space Chats</strong>&nbsp;will show up here once you
            join or create one.
          </Trans>
        </Notice>
      )}
      {currentChannel && (!msgs || msgs.length === 0) && sidebarView === 'spaces' && (
        <Notice>
          <Trans t={t}>
            ...
            <strong className='text-primary'>{currentChannel.name}</strong>&nbsp;space
          </Trans>
          {currentChannel.description && (
            <p className='text-charcoal-1 font-normal mt-0.5'>{currentChannel.description}</p>
          )}
        </Notice>
      )}
      {conversations?.length === 0 && sidebarView === 'dms' && (
        <Notice>
          <Trans t={t}>
            <strong className='text-primary'>Direct Messages</strong>&nbsp; will show up here once
            you or somebody else sends you a private message
          </Trans>
        </Notice>
      )}
      {currentConversation && currentDms?.length === 0 && (
        <Notice>
          <Trans t={t}>
            ...
            <Identity {...currentConversation} />
          </Trans>
          <p className='text-charcoal-1 font-normal mt-0.5'>{t('Say "Hi..."')}</p>
        </Notice>
      )}
    </>
  );
