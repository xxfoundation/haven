...
const CreateChannelView: FC = () => {
  ...
  const [channelName, onChannelNameChange, { set: setChannelName, touched: nameTouched }] =
    useInput();
  const [channelDesc, onChannelDescChange, { set: setChannelDesc, touched: descTouched }] =
    useInput();
  ...
  const changePrivacyLevel = useCallback((level: PrivacyLevel) => {
  ...
  const validateName = useCallback(() => {
    if (!regex.test(channelName) && nameTouched) {
      setNameError(
        t(
          'The name must be between 3 and 24 characters inclusive. It can only include upper and lowercase Unicode letters, digits 0 through 9, and underscores (_).'
        )
      );
    } else {
      setNameError('');
  ...
  const descError = useMemo(() => {
    if (channelDesc.length > 144 && descTouched) {
      return t('Description has to be under 144 characters.');
    }
    return null;
  }, [channelDesc, descTouched, t]);

  const error = nameError || descError;
  ...
  return (
    <>
      ...
      <div className='flex justify-center mt-8 w-full items-center'>
        <div className='rounded-full bg-charcoal-3 flex h-10 relative'>
          <div
            className={cn('absolute h-10 rounded-full bg-primary w-28 transition-all', {
              'left-0': privacyLevel === PrivacyLevel.Public,
              'left-28': privacyLevel === PrivacyLevel.Secret
            })}
          />
          <Button
            onClick={() => changePrivacyLevel(PrivacyLevel.Public)}
            className={cn('min-w-0 w-28 px-8 relative z-10 transition-all font-semibold', {
              'text-near-black': privacyLevel === PrivacyLevel.Public
            })}
            variant='unstyled'
          >
            {t('Public')}
          ...
          <Button
            onClick={() => changePrivacyLevel(PrivacyLevel.Secret)}
            className={cn('min-w-0 w-28 px-8 relative z-10 transition-all font-semibold', {
              'text-near-black': privacyLevel === PrivacyLevel.Secret
            })}
            variant='unstyled'
          >
            {t('Secret')}
      ...
      <Button data-testid='create-new-channel-button' className={cn('w-full')} onClick={onCreate}>
        {t('Create Space')}
      </Button>
    </>
  );
