...
const useGoogleRemoteStore = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      bus.removeListener(AppEvents.GOOGLE_TOKEN, setAccessToken);
    };
  }, []);
  ...
  const getBinaryFile = useCallback(
    async (name: string) => {
      const prefixed = prefixFilename(name);
  ...
  const readDir = useCallback(
    async (folderName: string): Promise<string[]> => {
      ...
      const suffixedFoldername =
        prefixedFolderName.charAt(prefixedFolderName.length - 1) === '/'
          ? prefixedFolderName
          : `${prefixedFolderName}/`;

      const filenames = filenamesResult.result.files.map((file: any) => prefixFilename(file.name));
