...
const useEvents = () => {
  ...
  const onAdminKeysUpdate = useCallback(
    (evt: AdminKeysUpdateEvent) => {
  ...
  const onDmTokenEnabledUpdate = useCallback(
    (e: ChannelDMTokenUpdate) => {
  ...
  const updateDmNotifications = useCallback(
    (e: DMNotificationsUpdateEvent) => {
  ...
  const onUserBlockUpdate = useCallback(
    (evt: DMBlockedUserEvent) => {
      if (evt.blocked) {
