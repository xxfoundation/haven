  ...
  const set = useCallback(
    async (v: T) => {
      if (kv) {
        ...
        throw new Error(`Attempted to set value on key ${key} but the store wasn't initialized`);
      }
    },
    [key, kv]
  );
...
export const useRemotelySynchedString = (key: string, defaultValue?: string) =>
  useRemotelySynchedValue(key, stringDecoder, defaultValue);
