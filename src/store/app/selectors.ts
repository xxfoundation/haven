...
export const messageDraft = (channelId?: ChannelId) => (state: RootState) =>
  (channelId && state.app.messageDraftsByChannelId[channelId]) ?? '';
export const channelFavorites = (state: RootState) => state.app.channelFavorites;
export const isChannelFavorited = (channelId?: ChannelId) => (state: RootState) =>
  state.app.channelFavorites.includes(channelId ?? '');
export const currentChannelOrConversationId = (state: RootState) =>
  state.app.selectedChannelIdOrConversationId;
...
export const highlighted = (messageId: string) => (state: RootState) =>
  state.app.highlightedMessageId === messageId;
