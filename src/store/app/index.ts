...
const slice = createSlice({
  ...
  reducers: {
    notifyNewMessage: (
      state: AppState,
      { payload: message }: PayloadAction<LastSeenMessagePayload>
    ) => {
    ...
    highlightMessage: (
      state: AppState,
      { payload: messageId }: PayloadAction<string | undefined>
    ) => ({
      ...state,
    ...
    dismissNewMessages: (
      state: AppState,
      { payload: channelId }: PayloadAction<ChannelId | ConversationId>
    ) => ({
      ...state,
      missedMessages: {
        ...state.missedMessages,
    ...
    updateContributorsSearch: (
      state: AppState,
      { payload: contributorsSearch }: PayloadAction<string>
    ) => ({
      ...state,
      contributorsSearch
    }),
    updateChannelsSearch: (
      state: AppState,
      { payload: channelsSearch }: PayloadAction<string>
    ) => ({
      ...state,
      channelsSearch
    }),
