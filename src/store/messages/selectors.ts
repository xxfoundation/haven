...
export const sortedMessagesByChannelId = (state: RootState) =>
  state.messages.sortedMessagesByChannelId;
...
export const reactionsTo = (message: Message) =>
  createSelector(reactions, dmReactions, (allReactions, allDmReactions) => {
    const channReactions = allReactions?.[message.id];
    const dmReacts = allDmReactions?.[message.id];
    return Object.entries(channReactions ?? dmReacts ?? {}).sort(
      (a, b) => b[1].length - a[1].length
    );
  });

export const repliedTo = (message: Message) =>
  createSelector(currentChannelMessages, currentDirectMessages, (msgs, dms) => {
    return (
      message.repliedTo &&
...
export const currentPinnedMessages = (state: RootState) =>
  currentChannelMessages(state)?.filter((msg) => msg.pinned);

export const currentChannelContributors = createSelector(
  currentChannelOrConversationId,
  contributors,
  (channelId, allContributors): Contributor[] =>
    channelId !== null && allContributors[channelId] ? allContributors[channelId] : []
);

export const messageableContributors = createSelector(contributors, (mapped) => {
  const flattened = flatten(Object.values(mapped)).filter((c) => c.dmToken !== undefined);

  return sortBy(
    uniqBy(flattened, (c) => c.pubkey),
    (c) => `${c.nickname?.toLocaleLowerCase() ?? ''}${c.codename.toLocaleLowerCase()}`
  );
});
...
export const commonChannels = (pubkey: string) => (state: RootState) =>
  state.messages.commonChannelsByPubkey[pubkey] || [];
