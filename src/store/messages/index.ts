...
const contributorMapper = (message: Message): Contributor =>
  pick(message, ['nickname', 'timestamp', 'pubkey', 'codeset', 'codename', 'color', 'dmToken']);
...
const commonChannelsReducer = (
  state: MessagesState['commonChannelsByPubkey'],
  message: Message
) => {
  return {
    ...state,
...
export const slice = createSlice({
  ...
  name: 'messages',
  reducers: {
    upsert: (state: MessagesState, { payload }: PayloadAction<Message>) =>
      messageReducer(state, payload),
    upsertMany: (state: MessagesState, { payload: messages }: PayloadAction<Message[]>) =>
      messages.reduce(messageReducer, state),
    delete: (
      state: MessagesState,
      { payload: messageId }: PayloadAction<MessageId>
    ): MessagesState => {
      let found: Message | undefined;
      Object.values(state.byChannelId).some((channelMessages) => {
      ...
      return {
        ...
        ...(found && {
          sortedMessagesByChannelId: {
            ...state.sortedMessagesByChannelId,
            [found.channelId]: state.sortedMessagesByChannelId[found.channelId].filter(
              (m) => m.uuid !== found?.uuid
            )
          },
          byChannelId: {
