...
export const currentConversation = (state: RootState): Conversation | null =>
  state.dms.conversationsByPubkey[state.app.selectedChannelIdOrConversationId ?? ''] || null;
export const conversationsByPubkey = (state: RootState) => state.dms.conversationsByPubkey;
export const conversations = createSelector(conversationsByPubkey, (byPubkey) =>
...
export const sortedDmsByPubkey = createSelector(allDms, (dms) =>
  mapValues(dms, (dmMap) => Object.values(dmMap).sort(byTimestamp))
);

export const searchFilteredConversations = (favorites: string[] = []) =>
  createSelector(conversations, dmsSearch, (convos, search) => {
    const sorted = sortBy(
      convos,
      (c) => (c.nickname ?? '').concat(c.codename.toLocaleLowerCase()),
      ['asc']
    );
...
export const isBlocked = (pubkey: string) => (state: RootState) =>
  blockedUsers(state).includes(pubkey);
export const notificationLevel =
  (pubkey?: string | null) =>
  (state: RootState): DMNotificationLevel | undefined =>
    pubkey ? state.dms.notificationLevels[pubkey] : undefined;
